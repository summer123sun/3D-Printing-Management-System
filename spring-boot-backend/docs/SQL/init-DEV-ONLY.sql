...
--
-- ⚠️⚠️⚠️  仅供本地开发 / 答辩演示使用  ⚠️⚠️⚠️
-- ⚠️⚠️⚠️  禁止在生产环境执行此 SQL       ⚠️⚠️⚠️
--
-- 风险：
--   1. 30 个测试账号统一弱口令 123456（含社长 2023010001 / 技术骨干 2023010002）
--   2. 手机号 / 邮箱占位数据是真实可注册号段
--   3. 数据量仅 30 账号 + 12 作品 + 4 项目，不适合生产
--
-- 生产环境请使用 init-prod-schema-only.sql（v2.12+ 提供），仅建表不灌数据
-- 由管理员通过 admin 端 addMember 一个个加真实成员
--
